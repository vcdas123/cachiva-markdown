...
Notes must be complete Markdown (`.md`) documents that follow every rule in `MARKDOWN_RULES` — Cachiva rejects invalid notes. A concise preamble paragraph between the H1 title and first H2 section is mandatory because it becomes the note description shown on cards. Every fenced code block must declare its language (` ```sql`, ` ```python`, etc., or ` ```text` if none applies); a bare fenced code block is a blocking validation error.
