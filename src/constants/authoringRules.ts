...
export const CODE_FENCE_RULE =
  "Every fenced code block MUST declare its language/type after the opening backticks. Use the actual language when known, e.g. ```sql, ```python, ```dockerfile, or ```bash. Use ```mermaid for Mermaid diagrams, which Cachiva renders visually in the reader. If no specific language applies, use ```text. NEVER output a bare fenced code block without a type.";
